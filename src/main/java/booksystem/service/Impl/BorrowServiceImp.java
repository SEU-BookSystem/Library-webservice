...
public class BorrowServiceImp implements BorrowService {
    ...
    public int cancelReserve(String lend_id) {
        //将对应的状态改为5.预约失败
        borrowDao.updateStatus(lend_id,5);
        return 0;
    }
    ...
    //收藏夹预约
    @Override
    public int addCollectionReserve(List<String> reference_nums, String user_id) {
        //首先查询用户现在借阅书籍（已借未逾期+逾期）的数量  <7本
        int sum = borrowDao.getBorrowNumByUser(user_id);
        if(sum<7) {
            //书籍要求：在馆且无人借阅无人预约
            for (String reference_num : reference_nums) {
                ...
                if(flag)
                {
                    //预约成功
                    //生成预约记录信息 状态为4.预约成功
                    Borrow borrow = new Borrow(null, user_id, null, null, bar_code,
                            0, 0, 4, null);
                    ...
                    //预约失败
                    //生成预约信息 状态为5.预约失败
                    Borrow borrow = new Borrow(null, user_id, null, null,
    ...
    //直接预约
    @Override
    public int addDirectReserve(String reference_num, String user_id) {
        //首先查询用户现在借阅书籍（已借未逾期+逾期）的数量  <7本
        int sum = borrowDao.getBorrowNumByUser(user_id);
        if (sum < 7) {
            //书籍要求：在馆且无人借阅无人预约
    ...
    public int borrowBook(int bar_code, String user_id) {
        ...
        if(book.get("status").equals(2)) {
            ...
            int sum = borrowDao.getBorrowNumByUser(user_id);
            Map<String,Object> overtime=borrowDao.getOvertimeBorrow(user_id);
            //借阅数量小于7本
            if(sum<7) {
                //没有逾期记录
                if(overtime.isEmpty()) {
    ...
    //还书
    @Override
    public int lendBook(int bar_code, String user_id){
        Map<String,Object> book=borrowDao.getBorrowByBarCode(bar_code);
        //将借阅状态更改为3.已还
        borrowDao.updateStatus(book.get("lend_id").toString(), 3);
        //更新书籍状态为1.在库
